jQuery(function($){
    ...
    $("div#pfptop").add("div#pfpback").click(function(){
        ...
        if (window.location.search.indexOf('view=internal') > -1) {
                $(".recordinfotext .additional").show();
        }
    if (window.location.pathname.indexOf('/dla/ead/search.html') === 0) {
      $(".resultstitle > a[href^='ead.pdf']").attr('href', function (i, attr) {
        var prefix;
        if (window.location.host.match(/^syslsl01/)) {
          prefix = 'http://devdla.library.upenn.edu/dla/pacscl/ead.pdf?';
        } else {
          prefix = '../pacscl/ead.pdf?';
        }
        return prefix + attr.replace(/^.*[?&]id=EAD_(upenn_[a-z]+)_([^&]+).*$/, 
            function (match, g1, g2) {
              return 'id=PACSCL_' + g1.toUpperCase() + '_' + g2;
            });
      });
    }
    $("a[href^='ead.pdf?id=EAD_']").attr('href', function (i, attr) {
      var prefix;
      if (window.location.host.match(/^syslsl01/)) {
        prefix = 'http://devdla.library.upenn.edu/dla/pacscl/ead.pdf?';
      } else {
...
function setIframeWidth() {
	...
	if (GetIEVersion() > 0) { document.getElementById('iframe').width = 0; }
...
function fixMenuHeight() {
	if (!(GetIEVersion() > 0 && GetIEVersion() <= 6 )) {
...
var processTextRow = function(currentTextBox) {
    ...
    var currentField = $("#" + currentType + "_field_" + position + ' option:selected').val();
    var currentOperator = $('#' + currentType + '_operator_' + position + ' option:selected').val();

    //The rows with empty text fields are simply ignored:
    if ($(currentTextBox).val().length) {

        var currentSuboperator = $('#' + currentType + '_suboperator_' + position + ' option:selected').val();
